...
def filter_question(imgs, atoi):
    new_imgs = []
    for i, img in enumerate(imgs):
        if atoi.get(img['ans'],10000) != 10000:
            new_imgs.append(img)
...
def main(params):
    ...
    train_image_ids = get_image_ids(imgs_train)
    test_image_ids = get_image_ids(imgs_test)
    ...
    print("ques_train", len(ques_train))
    print("ques_length_train", len(ques_length_train))
    print("answers", len(A))
    print("question_id_train", len(question_id_train))
    ...
    # f.create_dataset("train_img_ids", dtype='uint32', data=train_image_ids)

    data_train = {}
    data_train["questions"] = ques_train
    data_train["questions_length"] = ques_length_train
    data_train["answers"] = A
    data_train["questions_id"] = question_id_train
    data_train["images_pos"] = img_pos_train
    data_train["images_id"] = train_image_ids

    print("ques_test", len(ques_test))
    print("ques_length_test", len(ques_length_test))
    print("question_id_test", len(question_id_test))
    print("img_pos_test", len(img_pos_test))
    print("MC_ans_test", len(MC_ans_test))
    ...
    # f.create_dataset("test_img_ids", dtype='uint32', data=test_image_ids)
    data_test = {}
    data_test["questions"] = ques_test
    data_test["questions_length"] = ques_length_test
    data_test["mc_answers"] = MC_ans_test
    data_test["questions_id"] = question_id_test
    data_test["images_pos"] = img_pos_test
    data_test["images_id"] = test_image_ids
    ...
    with open(params['output_h5'], "wb") as f:
        pickle.dump(data, f)
    ...
    out = {}
    print("question vocab :",len(itow))
    print("answer ",len(itoa))
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_train_json', default="vqa_raw_train_train_val.json", help='input json file to process into hdf5')
    parser.add_argument('--input_test_json', default="vqa_raw_test_test.json", help='input json file to process into hdf5')
    parser.add_argument('--num_ans', default=3000, type=int, help='number of top answers for the final classifications.')
    parser.add_argument('--output_json', default='3000_data_prepro.json', help='output json file')
    parser.add_argument('--output_h5', default='3000_data_prepro.pkl', help='output h5 file')
    ...
    parser.add_argument('--token_method', default='token', help='token method. set "spacy" for unigram paraphrasing')
