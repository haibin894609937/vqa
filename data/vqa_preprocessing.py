...
def main(params):
    ...
    if params['split'] == 1:
        ...
        train_anno = json.load(open(path+'Annotations/mscoco_train2014_annotations.json', 'r'))
        val_anno = json.load(open(path+'Annotations/mscoco_val2014_annotations.json', 'r'))

        train_ques = json.load(open(path+'Questions/MultipleChoice_mscoco_train2014_questions.json', 'r'))
        val_ques = json.load(open(path+'Questions/MultipleChoice_mscoco_val2014_questions.json', 'r'))
        ...
        for i in range(len(train_anno['annotations'])):
            ...
            train.append({'ques_id': question_id, 'img_path': image_path,
                          ...
                          'question': question, 'MC_ans': mc_ans, 'ans': ans})
        ...
        for i in range(len(val_anno['annotations'])):
            ...
            test.append({'ques_id': question_id, 'img_path': image_path,
                         "image_id":val_anno['annotations'][i]['image_id'],
                        'question': question, 'MC_ans': mc_ans})
    elif params['split'] == 2:
        print 'Loading annotations and questions...'
        print("Loading ", path + 'Annotations/mscoco_train2014_annotations.json')
        print("Loading ", path + 'Annotations/mscoco_val2014_annotations.json')
        print("Loading ", path + 'Annotations/MultipleChoice_mscoco_test2015_questions.json')
        train_anno = json.load(open(path+'Annotations/mscoco_train2014_annotations.json', 'r'))
        val_anno = json.load(open(path+'Annotations/mscoco_val2014_annotations.json', 'r'))

        train_ques = json.load(open(path+'Questions/MultipleChoice_mscoco_train2014_questions.json', 'r'))
        val_ques = json.load(open(path+'Questions/MultipleChoice_mscoco_val2014_questions.json', 'r'))
        test_ques = json.load(open(path+'Questions/MultipleChoice_mscoco_test2015_questions.json', 'r'))
        ...
        print('Loading annotations and questions...')
        train_anno = json.load(open(path + 'Annotations/v2_mscoco_train2014_annotations.json', 'r'))
        val_anno = json.load(open(path + 'Annotations/v2_mscoco_val2014_annotations.json', 'r'))

        train_ques = json.load(open(path + 'Questions/v2_OpenEnded_mscoco_train2014_questions.json', 'r'))
        val_ques = json.load(open(path + 'Questions/v2_OpenEnded_mscoco_val2014_questions.json', 'r'))
        test_ques = json.load(open(path + 'Questions/v2_OpenEnded_mscoco_test2015_questions.json', 'r'))
    ...
    print('Training sample %d, Val sample %d Testing sample %d,...' % (len(train), len(val), len(test)))
    if params["split"] ==1:
        filename_train = 'vqa_raw_train_train.json'
        filename_test = 'vqa_raw_test_val.json'
    elif params["split"] ==2:
        filename_train = 'vqa_raw_train_train_val.json'
        filename_test = 'vqa_raw_test_test.json'
    else:
        filename_train = 'vqa_raw_train.json'
        filename_val = 'vqa_raw_val.json'
        filename_test = 'vqa_raw_test.json'
    json.dump(train, open(filename_train, 'w'))
    # json.dump(val, open(filename_val, 'w'))
    json.dump(test, open(filename_test, 'w'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default="/home/hbliu/dataset/mscoco/", type=str,
                        help='coco')
